import { get_dynamic_analysis_result } from "@/api"

export const useDynamicAnalysisStore = defineStore('dynamicAnalysis', () => {
    ...
    function pollTaskStatus(resultRef, interval = 2000) {
        ...
        resultRef.timerId = setInterval(async () => {
            // 轮询任务状态的逻辑
            try {
                const response = await get_dynamic_analysis_result(resultRef.task_id)
    ...
    persist: {
        enabled: true,
        strategies: [
            { key: 'dynamicAnalysis', storage: localStorage, paths: ['taskList', 'resultList'] }
        ]
    }
});
